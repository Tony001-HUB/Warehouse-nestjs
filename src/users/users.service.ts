...
<<<<<<< HEAD
  ...
  public async createUsers(newUser: UsersDto) {
    if (
      (this.availableUsers.includes(newUser.creatorInfo.role) &&
        this.getUserByEmail(newUser.creatorInfo.email)) ||
      (await this.getAllUsers()).length === 0
    ) {
      return await this.userRepository.create(newUser);
    } else {
      throw new HttpException(
        'There is no such user or there is no access.',
